...
fnc_avoid_screamer ={
	...
	_chased_units = _danger_close nearEntities ['Exile_Unit_Player',100];
	{
...
{	
	...
	{
		private ["_press_implicit_y","_press_implicit_x","_poz"];
		if (count (_entitate nearEntities ["Man",150])<2) then {_teleport=true;};
		_pot_tgt = _entitate nearEntities ['Exile_Unit_Player',150];
